...
var onoff = JSON.parse(window.localStorage.getItem("AutoJoomerOnoff"));
...
chrome.runtime.onInstalled.addListener(function (details) {
  ...
  if (details.reason == "install") {
    ...
    if (details.previousVersion != thisVersion)
      $.getJSON(
        "https://www.vishal-lokare.co/AutoJoomer/version_updates.json",
        function (ver) {
          var newUpdate = ver[thisVersion];
          window.alert(
            "AutoJoomer updated from " +
...
function runningscript() {
  ...
  const firebaseConfig = {
    ...
    databaseURL:
      "https://autojoomer-45dc2-default-rtdb.asia-southeast1.firebasedatabase.app",
    ...
    if (thatDay != null) {
      ...
      for (let i = 1, j = 0; i < nooflec; i++, j++) {
        ...
        if (
          thatClassName[i] == "" ||
          thatClassLink[i] == "" ||
          thatClass["class_time"] == ""
        )
          continue;
        if (!thatClassName[i] || !thatClassLink[i] || !thatClass["class_time"])
          continue;
        ...
        console.log(
          thatClassName[i] +
            " " +
            thatClassLink[i].substring(
              thatClassLink[i].length - 22,
              thatClassLink[i].length - 18
            ) +
            " " +
            h[i] +
            " " +
        ...
        if (millisOfThatClass[i] > 0) {
          timeouts[j] = setTimeout(function () {
            ...
            } else window.open(thatClassLink[i], "_blank");
          }, millisOfThatClass[i]);
